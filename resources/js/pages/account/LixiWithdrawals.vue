    ...
    <ClubMobileShell nav-active="account">
        ...
        <div class="space-y-5">
            <div>
                ...
                <p class="mt-1 text-center text-xs text-white">
                ...
                <p class="mt-2 text-center text-xs font-semibold text-white">
                    Điểm hiện có:
                    {{ user?.point ?? 0 }}
                </p>
            </div>
            ...
            <form class="rounded-2xl border border-pink-100 bg-pink-50/50 p-4 ring-1 ring-pink-100/80"
                @submit.prevent="submitWithdraw">
                <Label for="w-amt" class="text-neutral-800">Số điểm muốn rút</Label>
                <Input id="w-amt" v-model.number="form.amount" type="number" min="1" :max="user?.point ?? undefined"
                    class="mt-2 bg-white" inputmode="numeric" placeholder="Nhập số điểm" :disabled="!bankComplete" />
                ...
                <Button type="submit" class="mt-4 w-full bg-[#DA2778] hover:bg-[#b91560] text-white"
            ...
            <div>
                <h3 class="mb-3 text-center text-xs font-bold uppercase text-white">
                    Lịch sử rút
                </h3>
                <ul v-if="withdrawals.data.length" class="space-y-3">
                    <li v-for="w in withdrawals.data" :key="w.id"
                        class="rounded-2xl border border-pink-100 bg-white p-3 text-sm shadow-sm ring-1 ring-pink-50">
                        <div class="flex items-start justify-between gap-2">
                            <span class="font-semibold text-neutral-900">{{ w.amount }} điểm</span>
                        ...
                        <p v-if="w.admin_note && w.status === 'failed'" class="mt-2 text-xs text-neutral-700">
                            <span class="font-semibold">Ghi chú:</span>
                            {{ w.admin_note }}
                        </p>
                    </li>
                </ul>
                <p v-else class="text-center text-xs text-white/80">
                    Chưa có yêu cầu rút nào.
                </p>
            </div>

            <div v-if="withdrawals.links?.length > 3" class="flex flex-wrap justify-center gap-2">
                <a v-for="(l, i) in withdrawals.links" :key="i" :href="l.url || '#'" class="rounded px-2 py-1 text-xs"
                    :class="l.active
