    ...
    <ClubMobileShell nav-active="account">
        ...
        <div>
            ...
            <div class="mt-4 flex flex-wrap justify-center gap-2 text-xs font-semibold">
                <Link :href="tabHref('all')" class="rounded-full px-3 py-1.5 transition"
                    :class="filterTabClass(tab === 'all')">
                    Tất cả
                </Link>
                <Link :href="tabHref('spin')" class="rounded-full px-3 py-1.5 transition"
            ...
            <ul class="mt-6 space-y-3">
                <li v-for="t in transactions.data" :key="t.id"
                    class="rounded-2xl border border-pink-100 bg-white p-3 text-sm shadow-sm ring-1 ring-pink-50">
                    <div class="flex justify-between gap-2">
                        <span class="font-medium text-neutral-800">{{
                            typeLabel(t.type)
                        }}</span>
                        <span class="font-bold" :class="t.amount >= 0 ? 'text-green-600' : 'text-red-600'
                            ">
                    ...
                    <template v-if="t.type === 'wheel_spin' && t.meta">
                        <p class="mt-2 space-y-0.5 text-xs text-neutral-700">
                            <span v-if="t.meta.wheel_room_name" class="block text-neutral-600">
                            ...
                            <span v-if="t.meta.is_consolation" class="block text-neutral-600">
                                Ô an ủi — không đổi số dư điểm.
                            </span>
                            <span v-else class="block font-semibold text-green-700">
            ...
            <div v-if="transactions.links?.length > 3" class="mt-6 flex flex-wrap justify-center gap-2">
                <Link v-for="(l, i) in transactions.links" :key="i" :href="l.url || '#'"
